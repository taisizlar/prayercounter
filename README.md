# Prayer Counter

<div align="center">
  <img src="icon.png" width="100" />
  <br/>
  <img src="https://img.shields.io/badge/Android-21%2B-green?style=flat-square&logo=android" />
  <img src="https://img.shields.io/badge/Version-14.0-gold?style=flat-square" />

</div>

A simple, lightweight and accessible prayer counter (Tasbih) for Android.

## Features
- Count prayers with a single tap
- **Long press** the Add button to set a custom step — each tap will add that amount
- Save your count for later
- Works on lock screen
- Dark mode support (follows system theme)
- No unnecessary permissions — only vibration
- RTL layout support

## Download
[Releases](../../releases)

## Support
[@mahdi22mn](https://t.me/mahdi22mn)
